...
class AuthController(
    ...
    fun register(@RequestBody request: RegisterRequest): ResponseEntity<String> {
        val trimmedEmail = request.email.trim() // removes any hidden whitespace from the email
        val trimmedUsername = request.username.trim() // removes any hidden whitespace from the username

        if (userRepository.findByEmail(trimmedEmail).isPresent) { // check if email already exists in database
            return ResponseEntity.badRequest().body("This email is already registered") // block duplicate emails
        }
        if (userRepository.findByUsername(trimmedUsername).isPresent) { // check if username already exists in database
        ...
        val user = User(id, trimmedUsername, hashedPassword, salt, trimmedEmail, confirmationToken, false) // create user with trimmed values
        userRepository.save(user) // save user to database
        emailService.sendConfirmationEmail(trimmedEmail, confirmationToken) // send confirmation email in background
    ...
    fun login(@RequestBody request: LoginRequest): ResponseEntity<Any> {
        val trimmedInput = request.emailOrUsername.trim() // removes any hidden whitespace from the input

        val user = userRepository.findByEmail(trimmedInput) // try finding user by trimmed email first
            .orElseGet { userRepository.findByUsername(trimmedInput).orElse(null) } // fall back to trimmed username
    ...
    fun searchUsers(@RequestParam query: String): ResponseEntity<List<UserSearchResult>> {
        ...
        if (trimmedQuery.isEmpty()) { // reject empty queries
            return ResponseEntity.badRequest().build()
        }
        // searches for exact match on username or email only — no partial matches for privacy
        val results = userRepository.findByUsername(trimmedQuery)
            .map { listOf(UserSearchResult(it.id, it.username)) } // returns username only, no email shown
            .orElseGet {
                userRepository.findByEmail(trimmedQuery)
                    .map { listOf(UserSearchResult(it.id, it.username)) } // returns username only, no email shown
                    .orElse(emptyList()) // returns empty list if no exact match found
            }
...
// the response returned when searching for users — never includes email for privacy
data class UserSearchResult(
    val id: String,       // the user's unique id
    val username: String  // the user's username — email is intentionally excluded
)